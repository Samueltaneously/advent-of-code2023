...
function numberFinder(string) {
    ...
    const firstDigit = numbersInString[0];
    const lastDigit = numbersInString[numbersInString.length - 1];

    if (numbersInString.length === 1) {
        // If only one digit concatenate with itself
        return Number(`${firstDigit}${firstDigit}`);
    }

    if (firstDigit !== undefined && lastDigit !== undefined) {
        const stringNum = Number(`${firstDigit}${lastDigit}`);
        return stringNum;
    }
...
const solve = (data) => {
    for (const string of data) {
        const stringNum = numberFinder(string)
        console.log("number to add:", stringNum);
